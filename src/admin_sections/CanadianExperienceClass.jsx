...
const CanadianExperienceClassContent = () => {
  const notifySuccess = () => {
  ...
  const handleUpdateClick = () => {
    ...
    fetch(
      `https://brightlight-node.onrender.com/canadianExperienceClass/${sectionDataSingle._id}`,
      {
        method: "PUT",
        headers: {
  ...
  return (
    <div className={styles.singleSectionData}>
      ...
      <input
        placeholder="Cec Heading"
        name="CecHeading"
        value={sectionDataSingle.CecHeading || ""}
        onChange={handleInputChange}
        disabled={!editMode}
      />
      <textarea
        placeholder="Cec Paragraph"
        name="CecPara"
        value={sectionDataSingle.CecPara || ""}
        onChange={handleInputChange}
        disabled={!editMode}
      />
      ...
      {Array.from({ length: 3 }, (_, i) => (
        <textarea
          key={i}
          placeholder={`Still Not Sure Message ${i + 1}`}
          name={`s${i + 1}`}
...
export default CanadianExperienceClassContent;
